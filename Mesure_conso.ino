...
// 3 LED d'affichage 2 vertes, 1 rouge
// 2 boutons poussoir pour sélection gamme de mesure
// 1 switch 3 positions pour activation éclairage LCD et inhibition boutons poussoir
//
//--------------------------------------------------------------------------------------------------------------------------------
// Définitions bibliotèques
#include <Wire.h>
#include <TimeLib.h>
#include <MegaBox.h>  // bibliothèque pour HW MegaBox (carte Arduino Mega, afficheur LCD, LEDs, Switches)
...
String Date(unsigned long epoch, boolean Reverse = false); // fonction de conversion de date au format JJ/MM/AAAA ou inversé
...
const int RequestTimeMin = 1000;                                       // délai min entre 2 appuis boutons pour nouvelle prise en compte
boolean LCDon = true;                                                  // mémorise l'état ON/OFF de l'écran LCD
unsigned long TimeLastRequest = 0;                                     // mémorise l'instant de la dernière demande de changement de gain
...
void setup()
{
  InitSwitch();                          // init switches and buttons
  InitLed();                             // init sorties LED
                                         //
  ...
  InitLCD(); // initialise afficheur LCD
  ...
  if (WiFi.status() == WL_NO_MODULE)
  {
    ...
    while (true)
      ;
    { // bloque l'exécution sur cette ligne
      BlinkLED(RedLED);
    }
  }
  ...
  // Init serveur Web
  server.begin();
  Serial.print(F("Listening for clients on port "));
...
ISR(TIMER2_OVF_vect)
{
  ...
  while (ADCSRA & (1 << ADSC))
    ;                   //wait untill coversion be completed(ADSC=0);
  ...
  high = ADCH;
  if (high & (1 << 1))
  {                                    //in differential mode our value is between -512 to 511 (not 0 to 1023). it means we have 9 bits and 10th bit is the sign bit. but because
  ...
  if (varCompteur++ > SamplingCount)
  {                                   // traitement après N sampling
...
void loop()
{
  //
  while (!DataReady)
  { // boucle en attendant la disponibilité de data
    delay(100);
  }
  ...
  { // il est temps de vider le buffer vers la carte SD pour ne pas perdre de données
  ...
  if (TimeFromLastLog >= DatalogInterval)
  {                                             // il est temps d'écrire les données sur la carte SD
  ...
  // ------------------------------------------------------------------------
  // Gestion de la gamme de mesure (V8)
  //
  if (((millis() - TimeLastRequest) > RequestTimeMin) && LCDon)
  { // filte les demandes multiples
    // on ne traite les demandes éventuelles que si le LCD est allumé
    // on est sûr à ce stade qu'une demande n'était plus en cours
    if (IsClosed(PBblue))
    {
      // une demande d'augmentation de la sensibilté est détectée
      TimeLastRequest = millis(); // mémorise l'instant de la demande
      GainRequestUp = true;
      GainRequestDown = false;
    }
    if (IsClosed(PBred))
    {
      // une demande de diminution de la sensibilté est détectée
      TimeLastRequest = millis(); // mémorise l'instant de la demande
      GainRequestUp = false;
  ...
  {
    // si une demande était en cours elle a forcément été traité lors de l'interruption précédente
    // on peut donc les effacer
    GainRequestUp = false;
    GainRequestDown = false;

  } // fin gestion changement de gamme de mesure / sensibilité
  // ----------------------------------------------------------------
  // Gestion de l'éclairage LCD commandé par switch
  if (IsClosed(SWdown) && LCDon)
  {
    LCDon = false;
    lcd.noBacklight(); // éteint le retroéclairage LCD
    Serial.print("Etat switch : ");
    Serial.print(IsClosed(SWdown));
  }
  else if (!IsClosed(SWdown) && !LCDon)
  {
    LCDon = true;
  ...
  {
    ; // on ne fait rien car l'état du rétroéclairage est cohérent avec le switch
  }
  // fin de la gestion du rétroéclairage LCD
  //------------------------------------------------------------------------------------------------
  LastPower = Power;
  //
  // Traitement serveur
  ...
  } // fin de la gestion client Web
} // fin de la boucle principale (main)
//-----------------------------------------------------------------------
